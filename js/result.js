...
function calculateSimilarity(originalSrc, drawingSrc) {
    ...
    original.onload = function () {
        drawing.onload = function () {
            const score = compareForgivingSimilarity(original, drawing);
            finishResult(score);
        };
...
function compareForgivingSimilarity(original, drawing) {
    const size = 240;
    ...
    const maxGoodDistance = size * 0.28;
...
function getLooseOverlapScore(originalPixels, drawingPixels, size, tolerance) {
    let originalCount = 0;
    let matchedCount = 0;
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            ...
            if (originalPixels[index] !== 1) {
                continue;
            }

            originalCount++;

            if (hasNearbyPixel(drawingPixels, size, x, y, tolerance)) {
                matchedCount++;
            }
        }
    }
    ...
    return matchedCount / originalCount;
}
